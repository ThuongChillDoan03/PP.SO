...
# THUẬT TOÁN DÀNH CHO SỐ MỐC LẺ, CÁCH ĐỀU
...
def stirling(x, y):
    ...
    for i in range(1, n + 1):
        ...
        if i == 1:
            P1.append(np.array((Z1[i] + Z2[i]) * Li[i - 1] / (2 * giai_thua)))
        else:
            P1.append(np.concatenate((np.array([0]), P1[-1])) + (Z1[2*i - 1] + Z2[2*i - 1]) * Li[i - 1] / (2 * giai_thua))
        print("P1 =", P1[-1])
        ...
        if i == 1:
            P2.append(np.array((Z1[2*i] * Li[i - 1]) / giai_thua))
        else:
            P2.append(np.concatenate((np.array([0]),P2[-1])) + (Z1[2*i] * Li[i - 1]) / giai_thua)
        print("P2 =", P2[-1])
...
x, y = doc_input("Data_test_Stirling.xlsx")
kieu_moc, x, y = kiem_tra_input(x, y)
print(kieu_moc)
if(kieu_moc == 2 and kiem_tra_chan_le(x) == 1):
    h = x[1] - x[0]
    ...
    print("===============================================")
    ve_do_thi_da_thuc(P, x, y, kieu_moc)
