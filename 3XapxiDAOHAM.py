...
def forward_difference_first_order(data_x, data_y, x, dx):
    ...
    if idx[0].size != 0:
        if idx[0]+3 > len(data_x)-1:
            ...
            return (2*data_y[idx[0][0] + 3]-9*data_y[idx[0][0] + 2] + 18*data_y[idx[0][0] + 1] - 11*data_y[idx[0][0]])/(6 * dx)
    else:
...
def forward_difference_second_order(data_x, data_y, x, dx):
    ...
    if idx[0].size != 0:
        if idx[0]+3 > len(data_x)-1:
            ...
            return  (-data_y[idx[0][0] + 3] + 4 * data_y[idx[0][0] + 2] - 5 * data_y[idx[0][0] + 1] + 2 * data_y[idx[0][0]])/(dx * dx)
    else:
        print("Diem X khong thuoc cac moc noi suy da cho!")
...
def backward_difference_first_order(data_x, data_y, x, dx):
    ...
    if idx[0].size != 0:
        if idx[0] <= 2:
            ...
            return (11*data_y[idx[0][0]]-18*data_y[idx[0][0]-1] +9*data_y[idx[0][0] - 2] - 2*data_y[idx[0][0]-3])/(6 * dx)
    else:
        print("Diem X khong thuoc cac moc noi suy da cho!")
    ...
    if (len(data_x)-1<=3):
        ...
        if idx[0].size != 0:
            if idx[0]<=1 or idx[0]+2 > len(data_x)-1:
                ...
                return (-data_y[idx[0][0] + 2] + 8 * data_y[idx[0][0] + 1] - 8 * data_y[idx[0][0] - 1] + data_y[idx[0][0] - 2])/(12 * dx)
        else:
            print("Diem X khong thuoc cac moc noi suy da cho!")
    ...
    if (len(data_x)-1<=3):
        ...
        if idx[0].size != 0:
            ...
            if idx[0] <= 1 or idx[0] + 2 > len(data_x) - 1:
                ...
                return (-data_y[idx[0][0] + 2] + 16 * data_y[idx[0][0] + 1] - 30 * data_y[idx[0][0]] + 16 * data_y[idx[0][0] - 1] - data_y[idx[0][0] - 2])/(12 * dx * dx)
        else:
            print("Diem X khong thuoc cac moc noi suy da cho!")
